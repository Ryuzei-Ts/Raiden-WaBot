...
function setCachedMeta(groupJid: string, metadata: any) {
    ...
    setTimeout(() => {
        const c = groupMetaCache.get(groupJid);
        if (c && Date.now() - c.ts >= metaTtl) groupMetaCache.delete(groupJid);
    }, metaTtl);
}
...
export const handler = async (sock: any, rawMsg: any) => {
    try {
        ...
        const needsGroupMeta = msg.isGroup && (cmd.admin || cmd.botAdmin);
        const groupMetadata = needsGroupMeta ? await getGroupMetadata(sock, chat) : (msg.isGroup ? getCachedMeta(chat) : null);
