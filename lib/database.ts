...
const stmtGetUser = db.prepare('SELECT name FROM users WHERE id = ?');
...
const stmtGetChat = db.prepare('SELECT id FROM chats WHERE id = ?');
...
const stmtGetChatUser = db.prepare('SELECT messageCount FROM chat_users WHERE chat_id = ? AND user_id = ?');
const stmtInsertChatUser = db.prepare(`
...
`);
const stmtUpdateChatUser = db.prepare(`
    UPDATE chat_users 
    SET messageCount = messageCount + 1, lastSeen = ?
    WHERE chat_id = ? AND user_id = ?
`);

const processRegistration = db.transaction((sender: string, pushName: string, chat: string, now: number) => {
    const existingUser = stmtGetUser.get(sender) as { name?: string } | undefined;
    if (!existingUser) {
        stmtInsertUser.run(sender, pushName);
    } else if (pushName && existingUser.name !== pushName) {
