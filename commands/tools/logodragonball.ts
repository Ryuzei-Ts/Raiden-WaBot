...
export default {
    command: ['logodragonball', 'logodb', 'dblogo'],
    description: 'Crea un logo con efecto de texto estilo Dragon Ball',
    category: 'tools',
    group: true,
        ...
        try {
            ...
            if (!text) {
                return sock.sendMessage(chat, { 
                    text: `   ׄ  ✿  Por favor, ingresa un texto para crear el logo.\n\n> *Ejemplo:* ${p}logodb Ryuzei` 
                }, { quoted: m });
            }

            global.broadcast?.('cmd_progress', { id: msgId, step: 'generating_logo', text });

            const effectUrl = 'https://en.ephoto360.com/create-dragon-ball-style-text-effects-online-809.html';
            const endpoint = `https://api.delirius.online/tools/ephoto?effect=${encodeURIComponent(effectUrl)}&text=${encodeURIComponent(text)}`;
